...
static int episode_num;
static int selectedEpisode;
static bool newEpisodeSelected = false;

void setup() {
    Serial.begin(115200);
    Serial.println("Bagpuss Episode Player Starting");
...
void loop() 
{
  lv_timer_handler(); // Force LVGL to process the screen change
  delay(50); // Small delay to ensure screen is cleared

  if (newEpisodeSelected == true)
  {
    newEpisodeSelected = false;
...
// Manual deinitialization sequence
void shutdown_lvgl() {
  lv_init();
  lv_obj_del(lv_scr_act());  // Delete main screen
  lv_mem_deinit();  // Clear LVGL memory pools
  ...
  if(lv_disp_get_default()) {
    ...
    Serial.println("Completed LVGL shutdown routine");
    delay(500); 
}
...
void setupVideo() {
  ...
  avi_init();
  Serial.println("Completed video setup routine");
  delay(500); 
}
...
void playEpisode(uint8_t episodeNumber) {
  ...
  Serial.print("Preparing to play episode ");
  Serial.println(episodeNumber);
  ...
  if (!aviFile) {
    ...
    create_episode_selector(); // function to recreate the LVGL UI
}
    ...
    for(uint32_t i = 0; i < child_cnt; i++) {
        ...
        if (LV_ABS(diff_y) < 10) {
            ...
            sscanf(btn_text, "Episode %d", &episode_num);
             ...
             // Set the play episode flag
            newEpisodeSelected = true;
            selectedEpisode = episode_num;
            Serial.print("Prepare to play episode ");
            Serial.println(selectedEpisode);
